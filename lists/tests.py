...
class ListViewTest(TestCase):
	...
	def test_displays_all_items(self):
		...
		self.assertNotContains(response,'other list item 2') #self.assertContains no necesita contant.decode()
...
class NewListTest(TestCase):
	...
	def test_pagina_principal_redirige_despues_de_un_POST(self):
		...
		self.assertRedirects(response,'/lists/%d/' % (new_list.id,)) # assertRedirects: toma en cuenta la url completa
